...
namespace VPC.Framework.Business.MetadataManager.Data
{
    internal class LayoutData : EntityModelData
    {
        ...
        internal void UpdateLayoutDetailsXml(Guid tenantId, List<LayoutModel> templateModel)
        {

            try
            {
                var data =DataUtility.GetXmlForUpdateLayoutDetails(templateModel);

                
                SqlProcedureCommand cmd = CreateProcedureCommand("dbo.EntityLayout_Update_Xml");
                cmd.AppendGuid("@guidTenantId", tenantId);
                cmd.AppendXml("@xmlLayouts", DataUtility.GetXmlForUpdateLayoutDetails(templateModel));
                ExecuteCommand(cmd);
            }
            catch (SqlException e)
            {
                throw ReportAndTranslateException(e, "LayoutData::dbo.EntityLayout_Update_Xml");
            }
